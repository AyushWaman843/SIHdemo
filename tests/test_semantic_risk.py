...
class SemanticTests(unittest.TestCase):
    def test_http_failure_is_explained_without_raw_provider_body(self):
        response = requests.Response()
        response.status_code = 401
        with patch('backend.app.semantic_risk._call', side_effect=requests.HTTPError('secret provider body', response=response)):
            result = asyncio.run(assess_semantic('Welcome', enabled=True))
        self.assertEqual(result['ai_status'], 'failed')
        self.assertIn('HTTP 401', result['notice'])
